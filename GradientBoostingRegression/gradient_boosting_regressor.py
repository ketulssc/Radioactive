...
print("Practices under 5% Error: ",countVal)

# Create dictionary for storing result object 
dict = {'id': id, 'month': month, 'year': year, 'production' : pred_prod}
dp = pd.DataFrame(dict)

# Saving the dataframe to gradient_boosting_result.csv file
dp.to_csv('gradient_boosting_result.csv', index=False)
